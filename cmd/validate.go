...
func init() {
	...
	validateCmd.Flags().IntVarP(&step, "step", "s", 0, "Step number to validate (1-5)")
	validateCmd.Flags().BoolVarP(&allSteps, "all", "a", false, "Validate all steps")
}

func runValidate(cmd *cobra.Command, args []string) error {
	if !allSteps && (step < 1 || step > 5) {
		return fmt.Errorf("please specify a valid step number (1-5) or use --all flag")
	}
